...
namespace irb4400 {
  ...
  int BaseController<ControllerT, HardwareInterfaceT>::loadJointsAndSetJointHandlesMap() {
    ...
    kdl_getAllQNrFromTree(robot_state_data_.kdl_tree_, qnrs);
    ...
    for (auto&& name : joint_names) {
      try {
        ...
        robot_state_data_.joint_handle_info_.push_back(JointHandleInfo(q_nr, name, true, true));
    ...
    for (auto&& qnr : qnrs) {
      std::string joint_name = kdl_getJointNameFromQNr(robot_state_data_.kdl_tree_, qnr);
      JointHandleInfo jhi(qnr, joint_name, true, false);
  ...
  void BaseController<ControllerT, HardwareInterfaceT>::setControls() {
    handles_mutex_.lock();
      for (auto&& handle : joint_handles_map_) {
        ...
        //std::cerr << "set control: " << handle.first << ", " << u_(handle.first) << "\n";
      }
    handles_mutex_.unlock();
  }
